...
def find_from(board, word, y, x, seen):
    ...
    # This is called recursively to find smaller and smaller words until all tries are exhausted or successful.
    ...
    #Otherwise, this letter is good so note we've seen it and try all its neighbors for the first letter of the rest of the word
    print("%-6s%d,%d %-3s%-8s%-30s" % ("OK", y, x, board[y][x], word, seen))

    #This next line is tricky: we want to note we've seen the letter at this location, but we only want the child calls of this to get that and if we used `seen.add(...)` to add it to our set, *all* calls would get that, since the set is passed around that would mean that once we try a letter in one call, it would never be tried again, even in a totally different path. Therefore, we want to create a *new* seen set that ius equal to this set plus the new letter. Being a new object, rather than a mutated shared object, calls that don't descend from us won't have this `y, x` point in their seen. To do this we use the | (set-union) operator, read this line as "rebind seen to the union of the current seend and the set of point(y,x)). This could be written with an augmented operator as "seen |= {(y,x)}" in the same way "x = x + 2" can be written as "x += 2", but it would seem harder to understand.
    seen = seen | {(y, x)}
...
def find(board, word):
    ...
    #We've tried every path from every starting square without luck. Fail.
    return False
