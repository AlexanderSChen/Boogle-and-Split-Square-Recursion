...
def add(s1, s2):
    ...
    if type(s1) == type(s2) == int:
        # Return OR of squares: if either is filled in, it's filled in; otherwise, it's empty

        # Instead of using | operator, could do this directly with: return 0 if (s1 == 0 and s2 == 0) else 1
        return s1 | s2

    #Otherwise, if one is filled square and not a divided square, make a divided square out of it
    if type(s1) == int:
        s1 = [s1, s1, s1, s1]
    ...
    #zip(s1, s2) is nice -- it takes s1=[1, 1, 1, 1] and s1=[0, 0 ,0 ,0] and returns list of pairings [(1, 0), (1, 0), (1, 0), (1,0)]. Alternatively, you could do this with
