...
def validate(s):
    ...
    # base case: it's a simple square, so long as it's either 0 or 1 it's valid.
    if type(s) == int:
        return s == 0 or s == 1
    ...
    #It's a split square: Recurse intro quadrants and check each, "failing fast" 
    # Note: alternately, we could write the rest of this function in one pretty line by using the awesome `all(iterable)` function: return all(validate(q) for q in s)
    for q in s:
        if not validate(q):
            return False
    ...
    #a questionable style as it's less readable, but the entire function could be written as
    # return (
